...
# загружающий данные из обоих файлов и формирующий из них словарь: ключи — ФИО, значения —
...
with open('users.csv', 'r', encoding='utf-8') as users, open('hobby.csv', 'r', encoding='utf-8') as hobby:
    while True:
        name = users.readline()[:-1]
        hob = hobby.readline()[:-1]
        if name == '':
            if hob != '':
                exit(1)
            break
        name_hobby.setdefault(name)
        if hob != '':
            name_hobby[name] = hob.split(',')
        else:
            name_hobby[name] = None
