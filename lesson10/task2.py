...
for i in range(10):
    if randint(1, 2) == 1:
        Clothes_list.append(Suit(randint(1, 100)))
    else:
        Clothes_list.append(Coat(randint(1, 100)))

sum = 0
for i in range(10):
    print("%.2f" % Clothes_list[i].cloth_size)
    sum += Clothes_list[i].cloth_size
print(f'Всего необходимо: {"%.2f" %sum}')